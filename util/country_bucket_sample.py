...
class CountryBucketSample:
    """
    Sample with data grouped into buckets.
    """

    input_file: str = attr.ib(default="lag_sample", kw_only=True)
    """
    ...
    title: str = attr.ib(default="country_bucket_sample", kw_only=True)
    """
    ...
    def create_sample(self, *, caller_file: str) -> None:
        ...
        # The lists will hold data across all countries
        country_list = []
        x_bucket_mean_list = []
        y_bucket_mean_list = []
        y_bucket_std_list = []
        ...
        for country in countries:
            ...
            # Regression using buckets separately for each country
            for x_bucket_index in range(len(x_bucket_boundaries) - 1):
                ...
                bucket_filter = (country_df[x_feature] >= x_bucket_min) & (country_df[x_feature] < x_bucket_max)
                bucket_df = country_df[bucket_filter]
        ...
        sample_df.to_csv(f"{caller_name}.{self.title}.csv", index=False, float_format="%.6f")
