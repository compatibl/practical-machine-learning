...
if __name__ == "__main__":
    ...
    # Even countries have low reversion target, odd countries have high reversion target
    country_target = [0.05 if c % 2 == 0 else 0.15 for c in range(country_count)]

    # Perform simulation
    model = HullWhiteModel()
    model.year_count = 2000
    model.seed = seed
    ...
    # Create country bucket plot for even and odd countries separately
    country_bucket_plot_even = CountryBucketPlot()
    country_bucket_plot_even.x_feature = "short_rate(t)"
    country_bucket_plot_even.y_feature = "short_rate(t+5y)"
    country_bucket_plot_even.output_file = "country_bucket_plot.even"
    country_bucket_plot_even.countries = [model.countries[c] for c in range(0, country_count, 2)]
    country_bucket_plot_even.save_plot(caller_file=__file__)
    country_bucket_plot_odd = CountryBucketPlot()
    country_bucket_plot_odd.x_feature = "short_rate(t)"
    country_bucket_plot_odd.y_feature = "short_rate(t+5y)"
    country_bucket_plot_odd.output_file = "country_bucket_plot.odd"
    country_bucket_plot_odd.countries = [model.countries[c] for c in range(1, country_count, 2)]
    country_bucket_plot_odd.save_plot(caller_file=__file__)
